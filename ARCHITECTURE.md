...
Each profile has at most one optional `registry` connection. Its provider
defaults to Confluent and generated profiles persist the provider explicitly.
Confluent uses `schema.registry.url`; native Apicurio uses
`apicurio.registry.url`. The provider-specific properties are mutually
exclusive and accept only plain HTTP URLs.
...
A registry profile adds either `SCHEMA_REGISTRY_*` or `APICURIO_REGISTRY_*`;
inherited values from both families are cleared first.
...
receives a Confluent-compatible registry URL only when an Avro deserializer
requires `-r`. Official Kafka commands receive connection and properties-file
arguments. Confluent's schema-aware commands also require the Confluent
provider. Kaskade 5 `admin` and `consumer` receive a generated INI file, with
registry deserializers selecting a second provider-specific `[registry]` file;
Kaskade alone accepts native Apicurio profiles. Options that would override the
selected profile are rejected.
...
bootstrap servers. It requests `/subjects` for a Confluent-compatible registry
or `/search/artifacts` for native Apicurio. All operations use the configured
bounded timeout.
