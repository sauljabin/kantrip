    ...
    *,
    ...
    if shell_name == "zsh":
        ...
        contents = _render_shell_startup(
            original_startup,
            shim_directory,
            reset_zdotdir=environment.get("ZDOTDIR"),
            refresh_command="rehash",
        )
...
def _render_shell_startup(
    ...
    shim_directory: Path,
    *,
    reset_zdotdir: str | None = None,
    refresh_command: str,
) -> str:
    lines = ["# Generated by Kantrip for this profile session."]
    if reset_zdotdir is not None:
        lines.append(f"export ZDOTDIR={shlex.quote(reset_zdotdir)}")
    elif original_startup is not None and original_startup.name == ".zshrc":
